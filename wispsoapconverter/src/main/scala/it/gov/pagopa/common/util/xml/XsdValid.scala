...
import it.gov.pagopa.common.exception.XSDException
...
object XsdValid {
  ...
  def checkOnly(strXml: String, primitive: it.gov.pagopa.commonxml.XmlEnum.Value, toValid: Boolean = true): Try[Unit] = {
    if (toValid) {
      val loader: Array[Source] = it.gov.pagopa.commonxml.XmlEnum.loadSchema(primitive)
  ...
  def validate(input: String, sources: Array[Source], primitive: Option[it.gov.pagopa.commonxml.XmlEnum.Value] = None): Try[Unit] =
    Try({
