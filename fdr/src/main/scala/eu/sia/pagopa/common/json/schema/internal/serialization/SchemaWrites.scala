...
trait SchemaWrites { self: SchemaVersion =>
  ...
  def additionalWrites: OWrites[SchemaType] =
    OWrites[SchemaType](other => throw new RuntimeException(s"Unknown schema type encountered: $other"))
  ...
  lazy val refWrites: Writes[SchemaRef] = OWrites[SchemaRef] { ref =>
    Json.obj("$ref" -> ref.ref.value).deepMerge(anyConstraintWrites.writes(ref.constraints))
  }
  ...
  lazy val compoundWrites: Writes[CompoundSchemaType] = OWrites[CompoundSchemaType] { compound =>
    compound.alternatives.map(schemaTypeWriter.writes).foldLeft(Json.obj("type" -> Seq())) {
  ...
  val booleanWrites: Writes[SchemaBoolean] = OWrites[SchemaBoolean] { b =>
    Json.obj("type" -> "boolean").deepMerge(anyConstraintWrites.writes(b.constraints))
  }
  ...
  object Default {
    ...
    def integerWrites(numberConstraintWrites: OWrites[NumberConstraints]): OWrites[SchemaInteger] =
      OWrites[SchemaInteger] { i =>
    ...
    def numberWrites(numberConstraintWrites: OWrites[NumberConstraints]): OWrites[SchemaNumber] =
      OWrites[SchemaNumber] { num =>
