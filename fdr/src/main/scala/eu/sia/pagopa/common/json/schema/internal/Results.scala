...
object Results {
  ...
  def aggregateAsObject(validatedProps: Seq[(String, VA[JsValue])], context: SchemaResolutionContext): VA[JsValue] = {
    validatedProps.foldLeft[VA[JsValue]](Success(Json.obj()))((va, result) =>
      (va, result._2) match {
        case (Success(_), f @ Failure(_))              => f
        case (f @ Failure(_), Success(_))              => f
  ...
  def failureWithPath(keyword: String, msg: String, context: SchemaResolutionContext, instance: JsValue, additionalInfo: JsObject = Json.obj()): VA[JsValue] = {
