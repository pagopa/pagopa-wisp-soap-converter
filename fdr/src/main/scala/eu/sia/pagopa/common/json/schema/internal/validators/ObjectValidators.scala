...
import eu.sia.pagopa.common.json.schema.internal.{ Keywords, Props, Results, ValidationStep, ValidatorMessages }
import eu.sia.pagopa.common.json.schema.{ SchemaProp, SchemaResolutionContext, SchemaType, SchemaValue }
...
object ObjectValidators {
      ...
      val result: Seq[(String, VA[JsValue])] = propertyNames match {
        ...
        case Some(propertyNamesSchema) =>
          jsObject.fields.foldLeft(List.empty[(String, VA[JsValue])])((validatedFields, field) =>
            validatedFields :+ field._1 -> propertyNamesSchema
              .validate(JsString(field._1), context.updateScope(_.copy(schemaJsPath = context.schemaPath.map(_ \ "propertyNames"), instancePath = context.instancePath \ field._1)))
          )
      }
        ...
        json \ attr.name match {
          case _: JsUndefined =>
            if (required.contains(attr.name)) {
              attr.name ->
              Results.failureWithPath(Keywords.Object.Required, ValidatorMessages("obj.required.prop", attr.name), context, json) :: props
            } else {
              props
            }
          case JsDefined(value) =>
            (attr.name ->
            attr.schemaType.validate(value, context.updateScope(_.copy(schemaJsPath = context.schemaPath.map(_ \ "properties" \ attr.name), instancePath = context.instancePath \ attr.name)))) :: props
        }
      )

      val missing =
        required.filterNot(req => validated.exists(_._1 == req)).foldLeft(List.empty[(String, VA[JsValue])]) { (acc, req) =>
          json \ req match {
            case _: JsUndefined =>
              val result = req ->
                Results.failureWithPath(Keywords.Object.Required, ValidatorMessages("obj.required.prop", req), context, json)
              result :: acc
            case _ => acc
          }
        }
      ...
      val validated: Seq[(String, VA[JsValue])] = props.flatMap { prop =>
        {
          ...
          matchedPatternProperties.map(pp => {
            prop._1 -> pp._2.validate(prop._2, context.updateScope(_.copy(schemaJsPath = context.schemaPath.map(_ \ "properties" \ prop._1), instancePath = context.instancePath \ prop._1)))
          })
        }
      }
    ...
    def validateUnmatched(schemaType: SchemaType, context: SchemaResolutionContext): VA[JsValue] = {
      val validated = unmatchedFields.map { attr =>
        attr._1 -> schemaType.validate(
          attr._2,
          context.updateScope(_.copy(schemaJsPath = context.schemaPath.map(_ \ Keywords.Object.AdditionalProperties), instancePath = context.instancePath \ attr._1))
        )
      }
  ...
  def validateDependencies(schema: SchemaType, deps: Option[Map[String, SchemaType]], json: JsObject)(implicit lang: Lang): ValidationStep[Unit] = {
    ...
    def validatePropertyDependency(propName: String, dependencies: Seq[String], context: SchemaResolutionContext): VA[JsValue] = {
      ...
      val mandatoryProps = json.fields.find(_._1 == propName).map(_ => dependencies).getOrElse(Seq.empty[String])
        ...
        json.fields
          ...
          .fold(
            prop -> Results.failureWithPath(
              ...
              context.updateScope(_.copy(schemaJsPath = context.schemaPath.map(_ \ prop), instancePath = context.instancePath \ prop)),
              json
            )
      ...
      val updatedStatus = dependencies.foldLeft(status) { case (currStatus, dep) =>
        dep match {
          case (name, SchemaValue(JsArray(values))) =>
            ...
            val validated =
              validatePropertyDependency(name, values.toSeq.collect { case JsString(str) => str }, context)
      ...
      val result = maxProperties match {
        ...
        case Some(max) =>
          if (size <= max) Success(json)
            ...
            Results.failureWithPath(Keywords.Object.MaxProperties, ValidatorMessages("obj.max.props", size, max), context, json)
      }
      ...
      val result = minProperties match {
        ...
        case Some(min) =>
          if (size >= min) {
            ...
            Results.failureWithPath(Keywords.Object.MinProperties, ValidatorMessages("obj.min.props", size, min), context, json)
          }
      }
      ((), (), Results.merge(status, result))
    }
