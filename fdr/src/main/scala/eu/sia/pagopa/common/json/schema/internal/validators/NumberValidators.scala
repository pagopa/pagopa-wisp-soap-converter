...
import eu.sia.pagopa.common.json.schema.internal.constraints.Constraints.{ Maximum, Minimum }
...
object NumberValidators {
          ...
          minimum match {
            ...
            case Some(min) =>
              if (isValid(number, min)) {
                ...
                SchemaUtil.failure(Keywords.Number.Min, msg, context.schemaPath, context.instancePath, number)
              }
          }
          ...
          maximum match {
            ...
            case Some(max) =>
              if (isValid(number, max)) {
                ...
                SchemaUtil.failure(Keywords.Number.Max, msg, context.schemaPath, context.instancePath, number)
              }
          }
        case other => expectedNumber(other, context)
      }
          ...
          multipleOf match {
            case Some(factor) =>
              if (n.remainder(factor) == BigDecimal(0)) {
                ...
                SchemaUtil.failure(Keywords.Number.MultipleOf, ValidatorMessages("num.multiple.of", number, factor), context.schemaPath, context.instancePath, number)
              }
            case None => Success(number)
          }
        case other => expectedNumber(other, context)
  ...
  private def expectedNumber(json: JsValue, context: SchemaResolutionContext)(implicit lang: Lang) =
    SchemaUtil.failure(Keywords.Any.Type, ValidatorMessages("err.expected.type", "number", SchemaUtil.typeOfAsString(json)), context.schemaPath, context.instancePath, json)
          ...
          format match {
            ...
            case Some(schemaFormat) =>
              if (schemaFormat.validate(json)) {
                ...
                SchemaUtil.failure(Keywords.String.Format, ValidatorMessages("str.format", number, schemaFormat.name), context.schemaPath, context.instancePath, json)
              }
