package eu.sia.pagopa.common.json.schema.internal.refs

import com.osinka.i18n.Lang
import eu.sia.pagopa.common.json.schema._
import eu.sia.pagopa.common.json.schema.internal._
import eu.sia.pagopa.common.json.schema.internal.constraints.Constraints.Constraint
import eu.sia.pagopa.common.json.schema.internal.url.UrlStreamResolverFactory
...
/** Schema reference resolver.
  */
case class SchemaRefResolver(version: SchemaVersion, cache: DocumentCache = DocumentCache(), resolverFactory: UrlStreamResolverFactory = UrlStreamResolverFactory()) {
  ...
  /** Update the resolution scope based on the current element.
    ...
    * @param schema
    *   the value that might contain scope refinements
    * @return
    *   the updated scope, if the given value contain a scope refinement, otherwise the not updated scope
    */
  private[schema] def updateResolutionScope(scope: SchemaResolutionScope, schema: SchemaType): SchemaResolutionScope =
    schema match {
      case _ if refinesScope(schema) =>
        val updatedId = findScopeRefinement(schema).map(id => Refs.mergeRefs(id, scope.id, Some(resolverFactory)))
        scope.copy(id = updatedId)
      case _ => scope
    }

  /** Resolve the given ref against the current schema. The current schema must not contain
    *
    * @param current
    *   the current schema to resolve the ref against.
    * @param ref
    *   the ref to be resolved
    * @param scope
    *   the current resolution scope
    * @param lang
    *   the language to be used
    * @return
    *   the resolved schema together with the scope.
    */
  private[schema] def resolve(current: SchemaType, ref: Ref, scope: SchemaResolutionScope)(implicit lang: Lang): \/[JsonValidationError, ResolvedResult] = {
    ...
    if (scope.depth >= MaxDepth) {
      ...
      val result: \/[JsonValidationError, ResolvedResult] = ref match {
        ...
        case r if cache.contains(r) =>
          ...
          ResolvedResult(resolvedSchema, scope.copy(id = Some(Refs.mergeRefs(r, updatedScope.id)), documentRoot = resolvedSchema)).right[JsonValidationError]
        ...
        case a @ AbsoluteRef(absoluteRef) =>
          ...
          currentResolutionScope
            ...
            .map(remaining => resolve(current, Ref(if (remaining.startsWith("#")) remaining else "#" + remaining), updatedScope))
      ...
      result match {
        case \/-(resolvedResult @ ResolvedResult(resolved, _)) =>
          ...
          findRef(resolved).fold(result)(foundRef => resolve(resolvedResult.resolved, foundRef, resolvedResult.scope))
  ...
  private def resolveRelative(ref: RelativeRef, scope: SchemaResolutionScope, instance: SchemaType)(implicit lang: Lang): \/[JsonValidationError, ResolvedResult] = {
    Refs.mergeRefs(ref, scope.id) match {
      ...
      case r @ RelativeRef(relativeRef) =>
        ...
        val result = for {
          ...
          resolve(schema, LocalRef(localRef), scope.copy(documentRoot = schema, id = updateResolutionScope(scope, schema).id orElse Some(Ref(file)), referrer = scope.schemaJsPath))
        }
        result.getOrElse(resolutionFailure(r).left)
    }
  }

  private[schema] def resolveLocal(schemaPath: List[String], scope: SchemaResolutionScope, instance: SchemaType)(implicit lang: Lang): \/[JsonValidationError, ResolvedResult] =
    (schemaPath, instance) match {
      case (Nil, _) =>
        \/.fromEither(resolveSchema(instance, "").map(resolved => ResolvedResult(resolved, scope))) orElse ResolvedResult(instance, scope).right
      case (_, SchemaRef(ref, _, _)) if !ref.isInstanceOf[LocalRef] =>
        resolve(scope.documentRoot, ref, scope).flatMap(resolvedResult => resolveLocal(schemaPath, resolvedResult.scope, resolvedResult.resolved))
      case (schemaProp :: rest, resolvable) =>
        schemaProp match {
          case "#" => resolveLocal(rest, scope.copy(schemaJsPath = Some(JsPath \ "#")), scope.documentRoot)
          case _ =>
            \/.fromEither(resolveSchema(resolvable, schemaProp)).flatMap { r =>
              val newScope = updateResolutionScope(scope, r)
              resolveLocal(
                rest,
                newScope.copy(schemaJsPath =
  ...
  /** Fetch the instance located at the given URL and eventually cache it as well.
    *
    * @param ref
    *   the ref to fetch from
    * @param scope
    *   the current resolution scope
    * @return
    *   the fetched instance, if any
    */
    ...
    cache.get(ref.value) orElse cache.get(ref.value + "#") match {
      ...
      case _ =>
        for {
          url <- createUrl(ref)
          source <-
            if (url.getProtocol == null || version.options.supportsExternalReferences) {
  ...
  private[schema] def readJson(json: JsValue)(implicit lang: Lang): \/[JsonValidationError, SchemaType] =
    \/.fromEither(Json.fromJson[SchemaType](json).asEither).leftMap(errors => JsonValidationError(ValidatorMessages("err.parse.json"), JsError.toJson(errors)))
  ...
  /** Resolve the given ref. The given ref may be relative or absolute. If is relative it will be normalized against the current resolution scope.
    *
    * @param ref
    *   the ref to be resolved
    * @param scope
    *   the resolution scope that will be used for normalization
    * @return
    *   the resolved schema
    */
  private def resolveAbsolute(ref: AbsoluteRef, scope: SchemaResolutionScope)(implicit lang: Lang): \/[JsonValidationError, ResolvedResult] = {
    for {
      fetchedSchema <- fetch(ref.documentName, scope)
      result <- resolve(fetchedSchema, ref.pointer.getOrElse(Refs.`#`), scope.copy(id = Some(ref.documentName), documentRoot = fetchedSchema, referrer = scope.schemaJsPath))
    } yield result
  }

  /** Split the given ref into single segments. Only the fragments of the given ref will be considered.
    *
    * @param ref
    *   the reference that should be split up into single segments
    * @return
  ...
  private def splitFragment(ref: Ref): List[String] = {
    ...
    def escape(s: String): String =
      URLDecoder.decode(s, "UTF-8").replace("~1", "/").replace("~0", "~")

    ref.pointer.map(_.value).getOrElse(ref.value).split("/").toList.map(escape)
  }
  ...
  private def resolveConstraint[A <: Constraint](constraints: A, constraint: String)(implicit lang: Lang): Either[JsonValidationError, SchemaType] = {
    constraints.resolvePath(constraint).fold[Either[JsonValidationError, SchemaType]](Left(JsonValidationError(ValidatorMessages("err.unresolved.ref", constraint))))(schema => Right(schema))
  }

  private def findProp(props: Seq[SchemaProp], propName: String)(implicit lang: Lang): Either[JsonValidationError, SchemaType] = {
    props
      .collectFirst {
  ...
  private def findOtherProp(props: Seq[(String, SchemaType)], propName: String)(implicit lang: Lang): Either[JsonValidationError, SchemaType] = {
    props
      .collectFirst {
  ...
  def resolveSchema[A <: SchemaType](schema: A, fragmentPart: String)(implicit lang: Lang = Lang.Default): Either[JsonValidationError, SchemaType] = {
    ...
    schema match {
      ...
      case obj @ SchemaObject(props, _, otherProps) =>
        fragmentPart match {
      ...
      case arr @ SchemaArray(items, _, otherProps) =>
        fragmentPart match {
          case Keywords.Array.Items => Right(items)
          case other =>
            findOtherProp(otherProps, other).map(Right(_)).getOrElse(resolveConstraint(arr.constraints, fragmentPart))
        }
      ...
      case SchemaValue(value) =>
        (value, fragmentPart) match {
      ...
      case CompoundSchemaType(alternatives) =>
        val results = alternatives.map(alternative => resolveSchema(alternative, fragmentPart))
        results.collectFirst { case r @ Right(_) => r }.getOrElse(Left(JsonValidationError(ValidatorMessages("err.unresolved.ref", fragmentPart))))
      ...
      case SchemaNull(_)    => throw new RuntimeException("SchemaNull")
