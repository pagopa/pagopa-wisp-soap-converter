...
import eu.sia.pagopa.common.json.schema.{ SchemaInteger, SchemaNumber, SchemaResolutionContext, SchemaType, SchemaValue }
import play.api.libs.json.{ JsNumber, JsString, JsValue }
import scalaz.Success

case class NumberConstraints4(min: Option[Minimum] = None, max: Option[Maximum] = None, multipleOf: Option[BigDecimal] = None, format: Option[String] = None, any: AnyConstraints = AnyConstraints4())
    extends HasAnyConstraint
    with NumberConstraints {

  import eu.sia.pagopa.common.json.schema.internal.validators.NumberValidators._
  ...
  def isInt(implicit lang: Lang): scalaz.Reader[SchemaResolutionContext, Rule[JsValue, JsValue]] =
    scalaz.Reader { context =>
      Rule.fromMapping {
        ...
        case other =>
          SchemaUtil.failure(Keywords.Any.Type, ValidatorMessages("err.expected.type", "integer", SchemaUtil.typeOfAsString(other)), context.schemaPath, context.instancePath, other)
      }
    }
  ...
  override def validate(schema: SchemaType, json: JsValue, context: SchemaResolutionContext)(implicit lang: Lang): VA[JsValue] = {
    ...
    schema match {
      case SchemaInteger(_) =>
        isInt.flatMap(x => reader.map(y => x |+| y)).run(context).repath(_.compose(context.instancePath)).validate(json)
      case SchemaNumber(_) =>
