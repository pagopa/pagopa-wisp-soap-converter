...
object Refs {
  ...
  /** Normalizes a given ref, i.e. the result of this method is an always absolute URL wrapped in a ref, even if the given ref is relative.
    *
    * @param ref
    *   the ref to be normalized
    * @param currentScope
    *   the current resolution scope
    * @return
    *   an absolute URL wrapped in a ref
    */
    ...
    ref match {
      ...
      case r @ RelativeRef(relativeRef) =>
        if (relativeRef.startsWith("#") && currentScope.exists(_.value.startsWith("#"))) {
          ...
          ref
        } else if (relativeRef.startsWith("#")) {
  ...
  /** Finds out the actual base URL based on a given resolution scope.
    *
    * @param scope
    *   the resolution scope from which to determine the base URL
    * @return
    *   the base URL, if any, otherwise None
    */
...
object Ref {
  ...
  private def isAbsolute(value: String) = Try { new URI(value) }.map(_.isAbsolute).getOrElse(false)
