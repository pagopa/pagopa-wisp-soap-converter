...
import eu.sia.pagopa.common.json.schema.drafts.{ Version4, Version7 }
import eu.sia.pagopa.common.json.schema.internal._
import eu.sia.pagopa.common.json.schema.internal.refs.{ DocumentCache, Ref, SchemaRefResolver, SchemaResolutionScope }
...
class SchemaValidator(
  ...
  def addUrlHandler(handler: URLStreamHandler, scheme: String): SchemaValidator = {
    new SchemaValidator(schemaVersion, formats, resolverFactory.addUrlHandler(scheme, handler), cache)
  }
  ...
  def addSchema(id: String, schema: SchemaType): SchemaValidator = {
    new SchemaValidator(schemaVersion, formats, resolverFactory, cache.add(Ref(id))(schema).addAll(collectSchemas(schema, Some(Ref(id)))))
  }

  private def buildContext(refResolver: SchemaRefResolver, schema: SchemaType, schemaUrl: Option[Ref]): SchemaResolutionContext =
    SchemaResolutionContext(refResolver, SchemaResolutionScope(schema, schema.constraints.id.map(Ref(_)) orElse schemaUrl, Some(JsPath \ "#")), formats = formats)
  ...
  private def doValidate(schemaSource: Source, schemaUrl: Option[Ref] = None): JsValue => JsResult[JsValue] = {
    val context: JsResult[SchemaResolutionContext] = parseJson(schemaSource).toJsResult.flatMap { json =>
      ...
      val refResolver =
        SchemaRefResolver(version, schema.fold(_ => cache, s => cache.addAll(collectSchemas(s, id))), resolverFactory)
      schema.map(s => buildContext(refResolver, s, id))
    }
    input: JsValue => context.flatMap(ctx => ctx.scope.documentRoot.validate(input, ctx).toJsResult)
  }
  ...
  def validate[A](schemaSource: Source, input: => JsValue, reads: Reads[A]): JsResult[A] = {
    validate(schemaSource)(input).fold(valid = readWith(reads), invalid = errors => JsError(errors))
  }
  ...
  def validate[A](schemaUrl: URL, input: => JsValue, reads: Reads[A]): JsResult[A] = {
    validate(schemaUrl)(input).fold(valid = readWith(reads), invalid = errors => JsError(errors))
  }
  ...
  def validate[A: Format](schemaSource: Source, input: A): JsResult[A] = {
    ...
    validate(schemaSource, input, writes).fold(valid = readWith(reads), invalid = errors => JsError(errors))
  }
  ...
  def validate[A: Format](schemaUrl: URL, input: A): JsResult[A] = {
    val writes = implicitly[Writes[A]]
    val reads = implicitly[Reads[A]]
    validate(schemaUrl, input, writes).fold(valid = readWith(reads), invalid = errors => JsError(errors))
  }
  ...
  def validate(schema: SchemaType)(input: => JsValue): JsResult[JsValue] = {
    ...
    val context =
      SchemaResolutionContext(refResolver, SchemaResolutionScope(schema, ref, Some(JsPath \ "#")), formats = formats)
  ...
  def validate[A](schema: SchemaType, input: => JsValue, reads: Reads[A]): JsResult[A] = {
    ...
    result.fold(valid = readWith(reads), invalid = errors => JsError(errors))
  }
