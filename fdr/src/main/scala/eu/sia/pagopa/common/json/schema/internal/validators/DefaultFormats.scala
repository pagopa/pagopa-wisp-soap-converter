...
import eu.sia.pagopa.common.json.schema.SchemaFormat
  ...
  object DatetimeFormat extends SchemaFormat {
    ...
    override def validate(json: JsValue): Boolean = json match {
      case JsString(dateTime) =>
        \/.fromTryCatchThrowable[OffsetDateTime, DateTimeParseException](OffsetDateTime.parse(dateTime)).fold(_ => false, _ => true)
      case _ => false
    }
  }
