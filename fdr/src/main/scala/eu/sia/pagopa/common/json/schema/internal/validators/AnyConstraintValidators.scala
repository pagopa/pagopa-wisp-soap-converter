...
import eu.sia.pagopa.common.json.schema.{ SchemaObject, SchemaResolutionContext, SchemaType }
...
object AnyConstraintValidators {
  ...
  def validateIfThenElse(_if: Option[SchemaType], _then: Option[SchemaType], _else: Option[SchemaType])(implicit lang: Lang): scalaz.Reader[SchemaResolutionContext, Rule[JsValue, JsValue]] =
        ...
        (_if, _then, _else) match {
          case (Some(ifSchema), Some(thenSchema), Some(elseSchema)) =>
            ...
            if (ifSuccess && thenSchema.validate(json, context).isSuccess) {
              ...
              SchemaUtil.failure("else", ValidatorMessages("err.if.then.else", json), context.schemaPath, context.instancePath, json)
            }
          case (Some(ifSchema), Some(thenSchema), None) =>
            ...
            if (ifSuccess && thenSchema.validate(json, context).isSuccess) {
              ...
              SchemaUtil.failure("then", ValidatorMessages("err.if.then.else", json), context.schemaPath, context.instancePath, json)
            }
        ...
        not
          .map(schema =>
            if (schema.validate(json, context).isFailure) {
              ...
              SchemaUtil.failure(Keywords.Any.Not, ValidatorMessages("any.not", json), context.schemaPath, context.instancePath, json)
            }
          )
        ...
        allOf
          .map(schemas => {
            ...
            if (allMatch) {
              ...
              SchemaUtil.failure(Keywords.Any.AllOf, ValidatorMessages("any.all"), context.schemaPath, context.instancePath, json, collectFailures(allValidationResults, "/allOf"))
            }
          })
          .getOrElse(Success(json))
      }
  ...
  def validateAnyOf(schema: SchemaType, anyOf: Option[Seq[SchemaType]])(implicit lang: Lang): scalaz.Reader[SchemaResolutionContext, Rule[JsValue, JsValue]] = {
    ...
    @tailrec
    def untilFirstSuccess(json: JsValue, baseSchema: SchemaType, context: SchemaResolutionContext, schemas: List[SchemaType], results: List[VA[JsValue]]): List[VA[JsValue]] = schemas match {
      case s :: ss =>
          ...
          anyOf match {
            case Some(schemas) =>
              untilFirstSuccess(json, schema, context, schemas.toList, List.empty) match {
                ...
                case errors =>
                  SchemaUtil.failure(Keywords.Any.AnyOf, ValidatorMessages("any.any"), context.schemaPath, context.instancePath, json, collectFailures(errors, "/anyOf"))
              }
        ...
        oneOf
          .map(schemas => {
            ...
            allValidationResults.count(_.isSuccess) match {
              case 0 =>
                SchemaUtil.failure(Keywords.Any.OneOf, ValidatorMessages("any.one.of.none"), context.schemaPath, context.instancePath, json, collectFailures(allValidationResults, "/oneOf"))
              case 1 => Success(json)
              case _ =>
                ...
                SchemaUtil.failure(Keywords.Any.OneOf, ValidatorMessages("any.one.of.many"), context.schemaPath, context.instancePath, json, Json.obj("matched" -> matchedPaths))
            }
          })
          .getOrElse(Success(json))
      }
      ...
      Rule.fromMapping { json =>
        const match {
          ...
          case Some(constValue) =>
            SchemaUtil.failure("const", ValidatorMessages("any.const"), context.schemaPath, context.instancePath, json, Json.obj("const" -> constValue))
        }
      }
    }
  }
    ...
    scalaz.Reader { context =>
      ...
      Rule.fromMapping { json =>
        enums match {
          ...
          case Some(values) =>
            SchemaUtil.failure(Keywords.Any.Enum, ValidatorMessages("any.enum"), context.schemaPath, context.instancePath, json, Json.obj("enum" -> values))
          case None => Success(json)
        }
      }
    }
  ...
  private def collectFailures(results: Seq[VA[JsValue]], prefix: String): JsObject = {
    ...
    def repath(prefix: String)(obj: JsObject): JsObject = {
      ...
      val prefixedField = obj.fields.find(field => field._1 == "referrer").orElse(obj.fields.find(field => field._1 == "schemaPath")).map {
    ...
    results.zipWithIndex.foldLeft(Json.obj()) {
      case (obj, (Failure(errors), idx)) =>
        obj ++ Json.obj(
          s"$prefix/$idx" ->
          JsArray(SchemaUtil.toJson(errors).value.map {
