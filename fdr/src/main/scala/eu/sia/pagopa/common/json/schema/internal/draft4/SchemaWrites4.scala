...
import eu.sia.pagopa.common.json.schema.internal.serialization.SchemaWrites
import eu.sia.pagopa.common.json.schema.{ SchemaArray, SchemaInteger, SchemaNumber, SchemaObject, SchemaRoot, SchemaString, SchemaTuple, SchemaVersion }
import play.api.libs.json.{ Json, OWrites, Writes }

trait SchemaWrites4 extends SchemaWrites { self: SchemaVersion =>

  lazy val anyConstraintWrites: OWrites[AnyConstraints] = { case AnyConstraints4(schemaTypeAsString, allOf, anyOf, oneOf, definitions, enum, not, desc, id) =>
  ...
  lazy val objectConstraintWrites: OWrites[ObjectConstraints] = { case ObjectConstraints4(additionalProps, dependencies, maxProperties, minProperties, patternProps, required, any) =>
  ...
  lazy val numberConstraintWrites: OWrites[NumberConstraints] = { case NumberConstraints4(min, max, multipleOf, format, any) =>
    ...
    max.fold(emptyJsonObject)(max =>
      max.isExclusive match {
        case Some(isExclusive) =>
          Json.obj(Keywords.Number.Max -> max.max, Keywords.Number.ExclusiveMax -> isExclusive)
    ...
    min.fold(emptyJsonObject)(min =>
      min.isExclusive match {
        case Some(isExclusive) =>
          Json.obj(Keywords.Number.Min -> min.min, Keywords.Number.ExclusiveMin -> isExclusive)
